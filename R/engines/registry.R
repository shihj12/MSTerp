...
msterp_engine_registry <- function(force_rebuild = FALSE) {
  ...
  engines <- list(
    ...
    umap_gene = list(
      ...
      category = "trends",
      supported_data_types = c("proteomics", "metabolomics"),
      description = "Gene-level UMAP: one point per feature, embedded by similarity of its profile across samples. Optionally highlight a target list or k-means clusters.",
      supports_sequential = FALSE,
      accepted_input_levels = c("protein", "metabolite"),
      requirements = list(
        min_groups = 1,
      ...
      params_schema = list(
        msterp_schema_field("color_mode", "choice", "Color mode",
                            default = "none",
                            choices = c("none", "target_list", "cluster"),
                            choice_labels = c("None", "Target list", "K-means cluster"),
                            help = "How to colour points. 'Target list' highlights pasted IDs; 'K-means cluster' colours by k-means on the UMAP layout."),
        ...
        msterp_schema_field("scale_method", "choice", "Scaling",
                            default = "zscore",
                            choices = c("zscore", "pareto", "covariance", "none"),
                            help = "zscore: per-feature standardisation. pareto: divide by sqrt(SD). covariance: mean-center only. none: raw."),
        msterp_schema_field("missing_handling", "choice", "Missing value handling",
                            default = "drop_gene",
                            choices = c("drop_gene", "min_impute", "zero_impute"),
                            choice_labels = c("Drop feature", "Min-impute", "Zero-impute")),
      ...
      style_schema = c(
        list(
          ...
          msterp_schema_field("default_color", "string", "Default point colour",
                              ...
                              help = "Colour for non-highlighted / unclustered points."),
