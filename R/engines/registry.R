...
msterp_engine_registry <- function(force_rebuild = FALSE) {
  ...
  engines <- list(
    ...
    dsilac_isotope_density_peptide = list(
      ...
      category = "qc",
      supported_data_types = c("proteomics"),
      description = "Stacked density / histogram of light vs heavy isotope intensities, one row per sample, Light and Heavy as side-by-side columns (peptide level).",
      supports_sequential = FALSE,
      accepted_input_levels = c("peptide"),
      picker_hidden = TRUE,
      requirements = list(
      ...
      style_schema = list(
        ...
        msterp_schema_field("layout", "choice", "Layout", default = "separate",
                            ...
                            choice_labels = c("Overlay samples within Light/Heavy",
                                              "Stack samples as rows (Light | Heavy)"),
                            advanced = TRUE),
    ...
    dsilac_isotope_density_protein = list(
      ...
      category = "qc",
      supported_data_types = c("proteomics"),
      description = "Stacked density / histogram of light vs heavy isotope intensities, one row per sample, Light and Heavy as side-by-side columns (protein level).",
      supports_sequential = FALSE,
      accepted_input_levels = c("protein"),
      picker_hidden = TRUE,
      requirements = list(
    ...
    pca = list(
      ...
      style_schema = c(
        list(
          ...
          msterp_schema_field("show_group_labels", "bool", "Label groups on plot",
                              default = FALSE,
                              help = "Display group names at centroid of each group; hides legend when on"),
          msterp_schema_field("group_label_size", "num", "Group label size",
                              default = 5, min = 1, max = 15, advanced = TRUE),
          msterp_schema_field("group_label_color_mode", "choice", "Label color",
                              default = "group", choices = c("group", "flat"),
                              help = "Use each group's color, or a single flat color.",
                              advanced = TRUE),
          msterp_schema_field("group_label_flat_color", "string", "Label flat color (hex)",
                              default = "#000000", advanced = TRUE),
          msterp_schema_field("group_label_alpha", "num", "Label opacity",
                              default = 1, min = 0, max = 1, advanced = TRUE),
          msterp_schema_field("group_label_bg", "bool", "Label background",
                              default = FALSE, advanced = TRUE,
                              help = "Add a white background behind labels for readability"),
    ...
    umap = list(
      ...
      params_schema = list(
        ...
        msterp_schema_field("scale_method", "choice", "Scaling",
                            default = "zscore",
                            choices = c("zscore", "pareto", "covariance", "none"),
                            help = "zscore: per-feature standardisation. pareto: divide by sqrt(SD), dampens high-variance features. covariance: mean-center only. none: raw values."),
        ...
        msterp_schema_field("spread", "num", "UMAP spread",
                            default = 1, min = 0.5, max = 5, advanced = TRUE,
                            help = "Effective scale of embedded points; works with min_dist."),
        msterp_schema_field("n_epochs", "int", "UMAP n_epochs",
                            default = 0, min = 0, max = 1000, advanced = TRUE,
                            help = "0 = uwot picks (200/500). Higher = longer optimization."),
        msterp_schema_field("init", "choice", "Initialization",
                            default = "spectral",
                            choices = c("spectral", "normlaplacian", "random", "pca"),
                            advanced = TRUE),
      ...
      style_schema = c(
        list(
          ...
          msterp_schema_field("ellipse_level", "num", "Ellipse confidence",
                              default = 0.95, min = 0.5, max = 0.99, advanced = TRUE,
                              help = "Confidence level for the normal ellipse (groups with >=4 samples)."),
          msterp_schema_field("ellipse_alpha", "num", "Ellipse opacity",
                              default = 0.25, min = 0, max = 1, advanced = TRUE),
    ...
    umap_gene = list(
      engine_id = "umap_gene",
      label = "UMAP (gene-level)",
      category = "trends",
      supported_data_types = c("proteomics", "metabolomics"),
      description = "Gene-level UMAP: one point per melted feature/sample observation by default, or one point per feature profile when selected. Points can be colored by sample group, a target list, or k-means clusters.",
      supports_sequential = FALSE,
      accepted_input_levels = c("protein", "metabolite"),
      requirements = list(
        min_groups = 1,
      ...
      params_schema = list(
        msterp_schema_field("embedding_mode", "choice", "Point type",
                            default = "melted_observation",
                            choices = c("melted_observation", "feature_profile"),
                            choice_labels = c("Melted sample observations", "Feature profiles"),
                            help = "Melted sample observations creates one point per feature/sample value, similar to the Rmd-style plot. Feature profiles creates one point per feature using its full profile across samples."),
        msterp_schema_field("color_mode", "choice", "Color mode",
                            default = "sample_group",
                            choices = c("sample_group", "none", "target_list", "cluster"),
                            choice_labels = c("Sample group", "None", "Target list", "K-means cluster"),
                            help = "How to colour points. In melted mode, 'Sample group' uses the observation's sample group. In feature-profile mode, it uses the group with the highest processed mean value for each feature. 'Target list' highlights pasted IDs; 'K-means cluster' colours by k-means on the UMAP layout."),
        msterp_schema_field("target_list", "string", "Target list (one per line)",
                            default = "",
                            help = "Used when color_mode='target_list'. Paste IDs one per line (max 2000). Matches against feature ID and gene symbol."),
        msterp_schema_field("n_clusters", "int", "K-means clusters",
                            default = 6, min = 2, max = 30,
                            help = "Used when color_mode='cluster'."),
        msterp_schema_field("log_transform", "choice", "Log transform",
                            default = "log2", choices = c("log2", "log10", "none")),
        msterp_schema_field("scale_method", "choice", "Scaling",
                            default = "zscore",
                            choices = c("zscore", "pareto", "covariance", "none"),
                            help = "Melted mode scales observation-level numeric inputs. Feature-profile mode scales each feature across samples. zscore: standardise. pareto: divide by sqrt(SD). covariance: mean-center only. none: raw."),
        msterp_schema_field("missing_handling", "choice", "Missing value handling",
                            default = "drop_gene",
                            choices = c("drop_gene", "min_impute", "zero_impute"),
                            choice_labels = c("Drop feature", "Min-impute", "Zero-impute")),
        msterp_schema_field("melted_minimal_inputs", "bool", "Use minimal melted inputs",
                            default = TRUE,
                            help = "Melted-mode only. ON: embed on group_mean, group_delta, replicate_delta (3 cols) for crisper inter-group separation. OFF: use the full 7-column input (adds value, feature_mean, feature_sd, value_rank). Toggle to compare layouts."),
        msterp_schema_field("min_valid_fraction", "num", "Min fraction of valid samples",
                            default = 0.7, min = 0, max = 1, advanced = TRUE),
        msterp_schema_field("variance_filter_top_n", "int", "Keep top-N features by variance",
                            default = 1000, min = 100, max = 10000,
                            help = "Filters the feature pool before melting; in melted mode this caps how many observations are produced (top-N features x samples)."),
        msterp_schema_field("n_neighbors", "int", "UMAP n_neighbors",
                            default = 100, min = 2, max = 200,
                            help = "Lower = local structure; higher = global. Melted observation mode benefits from larger neighborhoods (30+) since point counts are typically large."),
        msterp_schema_field("min_dist", "num", "UMAP min_dist",
                            default = 0.01, min = 0, max = 0.99,
                            help = "Lower = tighter clusters. 0.01 gives crisp separation in melted mode."),
        msterp_schema_field("spread", "num", "UMAP spread",
                            default = 1, min = 0.5, max = 5, advanced = TRUE),
        msterp_schema_field("n_epochs", "int", "UMAP n_epochs",
                            default = 1000, min = 0, max = 1000, advanced = TRUE,
                            help = "0 = uwot picks (200/500). Higher = longer optimization, crisper convergence."),
        msterp_schema_field("init", "choice", "Initialization",
                            default = "spectral",
                            choices = c("spectral", "normlaplacian", "random", "pca"),
                            advanced = TRUE),
        msterp_schema_field("metric", "choice", "Distance metric",
                            default = "euclidean",
                            choices = c("euclidean", "correlation", "cosine", "manhattan"),
                            help = "Euclidean is recommended for melted observation mode (input is 7 scaled numeric features)."),
        msterp_schema_field("seed", "int", "Random seed",
                            default = 42, min = 0, max = 2147483647, advanced = TRUE)
      ),
      style_schema = c(
        list(
          msterp_schema_field("point_size", "num", "Point size",
                              default = 1.2, min = 0.3, max = 8, advanced = TRUE),
          msterp_schema_field("point_alpha", "num", "Point opacity",
                              default = 0.6, min = 0, max = 1, advanced = TRUE),
          msterp_schema_field("default_color", "string", "Default point colour",
                              default = "#888888", advanced = TRUE,
                              help = "Colour for uncolored points and non-target background points."),
          msterp_schema_field("target_color", "string", "Target highlight colour",
                              default = "#E74C3C", advanced = TRUE),
          msterp_schema_field("target_size_mult", "num", "Target size multiplier",
                              default = 2, min = 1, max = 6, advanced = TRUE,
                              help = "Targets are drawn larger than the background cloud."),
          msterp_schema_field("show_target_labels", "bool", "Label target points",
                              default = FALSE),
          msterp_schema_field("label_size", "num", "Label size",
                              default = 3, min = 1, max = 10, advanced = TRUE)
        ),
