...
stats_umap_run <- function(payload, params = NULL, context = NULL) {
  ...
  } else if (scale_method %in% c("zscore_group", "zscore_sample")) {
    mat <- nr_scale_within_samples(mat)
    add_log("INFO", "Applied per-sample z-score scaling (each column standardised)")
  } else {
