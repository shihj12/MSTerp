# =========================================================
# R/engines/stats/fc_rankplot.R — Fold Change Rank Plot Engine
#
# Computes per-feature log2 fold-change for every pairwise group
# comparison and ranks features descending by FC (rank 1 = highest FC).
# No statistical testing (no p-values, no padj, no significance class).
#
# Contract:
#  - data$comparisons: named list keyed by "B_vs_A"; each entry:
#    - points: primary_id, display_id, protein_id, gene_id,
#              log2fc, mean_a, mean_b, n_a, n_b, rank
...
.fc_rankplot_pairwise <- function(mat, samples, grp_a, grp_b,
                                  primary_ids, display_ids,
                                  fc_transform, is_log_transformed, add_log) {
  add_log("INFO", sprintf("--- FC Rank: %s vs %s ---", grp_a, grp_b))
  ...
  # Default rank: rank 1 = highest log2fc (descending). Renderer will recompute
  # after style-time min_replicates / flip_fc.
  if (nrow(results) > 0) {
    results$rank <- rank(-results$log2fc, ties.method = "average", na.last = "keep")
  } else {
    results$rank <- numeric(0)
  }
...
stats_fc_rankplot_run <- function(payload, params = NULL, context = NULL) {
  ...
  add_log("INFO", sprintf("Initiating fc_rankplot: %d features, %d groups",
  ...
  if (length(groups) < 2) {
    add_log("ERROR", "fc_rankplot requires at least 2 groups")
    return(list(
      engine_id = "fc_rankplot",
      params = params,
      data = list(comparisons = list(), comparison_names = character(),
                  fc_vectors = list(), log = build_log_df(),
                  error = "fc_rankplot requires at least 2 groups")
    ))
  }
  ...
  add_log("INFO", sprintf("fc_rankplot completed in %.2f seconds (%d comparisons)",
